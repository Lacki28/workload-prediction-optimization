...
def my_loss_fn(output, target):
    ...
    loss += naive_ratio(6, output, target)
    return loss
...
def main(t=1, sequence_length=12, epochs=2000, features=['mean_CPU_usage'], target=["mean_CPU_usage"],
    ...
    df = pd.read_csv("3418324.csv", sep=",")
...
if __name__ == "__main__":
 #   for history in (12):
    main(t=6, sequence_length=12, epochs=100, features=['mean_CPU_usage'], target=['mean_CPU_usage'], num_samples=2)
