...
class RegressionLSTM(nn.Module):
    def __init__(self, num_sensors, num_hidden_units, num_layers, t, dropout, lin_layers):
        ...
        # )
        self.fc_1 = nn.Linear(num_hidden_units, lin_layers)  # fully connected
        self.fc_2 = nn.Linear(lin_layers, 2)  # fully connected last layer
        self.relu = nn.ReLU()
    ...
    def forward(self, x):
        ...
        output, (hn, cn) = self.lstm(x, (h0, c0))  # (input, hidden, and internal state)
        output = output[:, -1, :]
        out = self.relu(output)
        out = self.fc_1(out)  # first dense
        out = self.relu(out)  # relu
        final_output = self.fc_2(out)  # final output
        final_out_cpu = final_output[:, 0]
        final_out_mem = final_output[:, 1]
...
def test_model(data_loader, model, optimizer, ix_epoch, device):
    ...
    with torch.no_grad():  # do not calculate the gradient
        for i, (X, y) in enumerate(data_loader):
            ...
            cpu, mem = model(X)
            loss_cpu += my_loss_fn(cpu, y[:, 0])
            loss_mem = my_loss_fn(mem, y[:, 1])
    ...
    tune.report(loss=((loss_mem + loss_cpu) / len(data_loader)), loss_cpu=(loss_cpu / len(data_loader)),
                loss_mem=(loss_mem / len(data_loader)), accuracy=(
                accuracy / len(
...
def calculate_prediction_results(t, pred_cpu_test, pred_mem_test, act_cpu_test, act_mem_test, pred_cpu_train,
    ...
    calc_MSE_Accuracy(t, act_mem_train, pred_cpu_train, file_path, start_time, training_time)
...
def train_and_test_model(config, checkpoint_dir="checkpoint", training_data_file=None, t=None, epochs=None,
    ...
    model = RegressionLSTM(num_sensors=len(features), num_hidden_units=config["units"], num_layers=config["layers"],
                           t=t, dropout=0, lin_layers=config['lin_layers'])
...
def main(t=1, sequence_length=12, epochs=2000, features=['mean_CPU_usage'], target=["mean_CPU_usage"],
         num_samples=100):
    file_path = 'lstm_filter_big_test.txt'
    ...
    reporter = CLIReporter(
        metric_columns=["loss","loss_cpu", "loss_mem", "accuracy", "training_iteration"])
    ...
    best_trained_model = RegressionLSTM(num_sensors=len(features), num_hidden_units=best_trial.config["units"],
                                        num_layers=best_trial.config["layers"], t=t, dropout=0, lin_layers=best_trial.config["lin_layers"])
    device = "cpu"
    if torch.cuda.is_available():
        device = "cuda:0"
...
if __name__ == "__main__":
    main(t=2, sequence_length=12, epochs=50, features=['mean_CPU_usage', 'canonical_mem_usage'],
    ...
    #         main(t=t, sequence_length=history, epochs=50, features=['mean_CPU_usage', 'canonical_mem_usage'],
    #              target=['mean_CPU_usage','canonical_mem_usage'],
    #              num_samples=1)
