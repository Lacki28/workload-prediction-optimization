...
def calculate_prediction_results(t, pred_cpu_test, pred_mem_test, act_cpu_test, act_mem_test, pred_cpu_train,
    ...
    for i in range(t):
        append_to_file(file_path, str(i) +" timestamp ahead prediction")
...
def plot_results(t, predictions_cpu, predictions_mem, actual_values_cpu, actual_values_mem, sequence_length, target,
    ...
    for i in range(t):
        ...
        plt.savefig('LSTM_multivariate_bi_directional_' + 'h' + str(sequence_length) + '_t' + str(i) + '' + '.png')
...
def main(t=1, sequence_length=12, epochs=2000, features=['mean_CPU_usage'], target=["mean_CPU_usage"],
         num_samples=100):
    file_path = 'lstm_multivariate_bidirectional.txt'
...
if __name__ == "__main__":
    # main(t=6, sequence_length=6, epochs=250, features=['mean_CPU_usage', 'canonical_mem_usage'],
    #      target=['mean_CPU_usage', 'canonical_mem_usage'],
    #      num_samples=16)
    for t in (3, 6):
        for history in (2,7):
            main(t=t, sequence_length=history, epochs=10, features=['mean_CPU_usage', 'canonical_mem_usage'],
                 target=['mean_CPU_usage', 'canonical_mem_usage'],
                 num_samples=1)
