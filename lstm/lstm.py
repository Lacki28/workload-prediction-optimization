...
# hyperparameters
sequence_length = 2  # I want to make a prediction based on how many values before
n = 1  # how many timestamps after I want to predict - example: n=1, sequ =3: x=[1,2,3],y=[4]
batch_size = 8  # batch_size = nr of sequences that are processed in parallel during training - gradients of the loss function are calculated based on the average loss over the entire batch.
learning_rate = 0.0005
num_hidden_units = 1  # The size of the LSTM layers, or the number of hidden units in each layer, will affect the capacity of the model. A larger number of hidden units will allow the model to capture more complex patterns in the data, but may also make the model more prone to overfitting.
num_layers = 1
epochs = 200
...
class RegressionLSTM(nn.Module):
    def __init__(self, num_sensors):
        ...
        self.linear1 = nn.Linear(num_hidden_units, 32)
        self.linear2 = nn.Linear(num_hidden_units, 32)
...
def calculate_prediction_results(df_train, df_out, prediction_train_cpu, prediction_train_mem, prediction_test_cpu, prediction_test_mem):
...
def plot_results(index_in_hours, df_train, df_test, prediction_test_cpu, prediction_train_cpu, prediction_test_mem,
    ...
    index_test = index_in_hours.iloc[len(df_train) + sequence_length:len(df_train) + len(df_test)]
    index_train = index_in_hours.iloc[sequence_length:len(df_train)]
...
def main():
    ...
    features = ['start_time', 'mean_CPU_usage', 'unmapped_page_cache_mem_usage',
                'unmapped_page_cache_mem_usage']  # ['start_time', 'mean_CPU_usage', 'unmapped_page_cache_mem_usage', 'unmapped_page_cache_mem_usage']
    ...
    model = RegressionLSTM(num_sensors=len(features))
    ...
    prediction_test = predict(test_eval_loader, model)
    ...
    calculate_prediction_results(df_train, df_out, prediction_train_cpu, prediction_train_mem, prediction_test_cpu, prediction_test_mem)
