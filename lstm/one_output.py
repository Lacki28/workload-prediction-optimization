...
def calc_MSE_Accuracy(y_test, y_test_pred, index):
    ...
    for i in range(len(y_test)):
        mae.append(round(sm.mean_absolute_error(y_test[i], y_test_pred[i]), 5))
        mse.append(round(sm.mean_squared_error(y_test[i], y_test_pred[i]), 5))
        r2.append(round(sm.r2_score(y_test[i], y_test_pred[i]), 5))
        nr.append(naive_ratio(y_test_pred[i], y_test[i]))
...
def plot_results(predictions, actual_values, original_test_files, config):
    ...
    fig, axs = plt.subplots(nrows=len(index_list), ncols=1, figsize=(10, 5))
    for i in range(len(index_list)):
        if len(index_list) == 1:
            axs.plot(index_list[i], actual_values, label='actual CPU usage', linewidth=1,
                        markerfacecolor='blue')
            axs.plot(index_list[i], predictions, label='predicted CPU', linewidth=1, markerfacecolor='red')
            axs.set_xlabel('Time (hours)')
            axs.set_ylabel('CPU prediction')
            axs.set_title('Mean CPU prediction')
            axs.legend()
        else:
            axs[i].plot(index_list[i], actual_values[i], label='actual CPU usage', linewidth=1,
                           markerfacecolor='blue')
            axs[i].plot(index_list[i], predictions[i], label='predicted CPU', linewidth=1, markerfacecolor='red')
...
def get_prediction_results(test_data_files_sequence, best_trained_model, device, config):
    ...
    for test_dataset in test_data_files_sequence:
        ...
        prediction_test_cpu = prediction_test_cpu.squeeze()
        prediction_test.append(denormalize_data_minMax(prediction_test_cpu))
    ...
    for sequence_file in test_data_files_sequence:
        ...
        actual_test_values.append(denormalize_data_minMax(actual_test_cpu))
...
def main():
    ...
    config = {
        "lin_layers": tune.choice([5, 10, 25]),
        "units": tune.choice([4, 6, 8]),
        "layers": tune.choice([1, 3, 4, 5]),
        "lr": tune.loguniform(0.0001, 0.0009),  # takes lower and upper bound
        "batch_size": tune.choice([8]),
        "sequence_length": tune.choice([1, 6, 12])  # , 6, 12, 24]),
    }
