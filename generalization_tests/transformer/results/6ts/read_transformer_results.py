...
def get_avg_loss(dir):
    ...
    for timestamp in range(6):
        ...
        with open('new_data_' + dir + str(timestamp + 1) + ".txt", 'r') as file:
            for line in file:
        ...
        with open('avg_' + str(timestamp + 1) + ".txt", 'a+') as file:
            file.write(
                "\multirow{2}{*}{Transformer} & \multirow{2}{*}{" + dir + "} & {avg} & \multirow{2}{*}"
                                                                          "{dm=32, nh=1, lr=4.04e-06, bs=128, df=16, nl=1 epochs=300} &\multirow{2}{*}{1}& " +
