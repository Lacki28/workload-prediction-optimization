...
def create_boxplot(loss, name, timestamp, dir):
    ...
    plt.title('ARIMA ' + name, fontsize=22)
...
def get_avg_loss(dir):
    ...
    for timestamp in range(6):
        ...
        with open('avg_' + str(timestamp + 1) + ".txt", 'a+') as file:
            file.write(
                "\multirow{2}{*}{ARIMA} & \multirow{2}{*}{" + dir + "} & {avg} & \multirow{2}{*}{individual} &\multirow{2}{*}{4}& " +
                calc_avg(mae) + " & " + calc_avg(mse) + " & " + calc_avg(r2) + " & " + calc_avg(nr) + " & " +
                "\multirow{2}{*}{" + calc_avg(training_time) + "}" + " & " "\multirow{2}{*}{" + calc_avg(
                    total_time) + "}\\\\\n")
            file.write(
                "&   & std & & &" + calc_std(mae) + " & " + calc_std(mse) + " & " + calc_std(r2) + " & " + calc_std(
                    nr) + " &  & \\\\\n")
